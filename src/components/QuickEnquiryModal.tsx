...
export const QuickEnquiryModal: React.FC<QuickEnquiryModalProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-lg max-h-[92vh] sm:max-h-[88vh] bg-white shadow-2xl border border-slate-200 flex flex-col my-auto overflow-hidden text-slate-800 animate-in fade-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Pinned Modal Header (Always visible) */}
        <div className="shrink-0 px-4 py-3.5 sm:px-6 sm:py-4 bg-[#08546C] text-white flex items-center justify-between border-b border-[#053D4F]">
          <div className="flex items-center space-x-3">
            <div className="bg-white px-2 py-1 shadow-xs">
              <img
            ...
            <div>
              <h3 className="text-sm sm:text-base font-bold text-white leading-tight">Workforce Consultation</h3>
              <p className="text-[10px] sm:text-[11px] text-cyan-200">Cadzook Private Limited &bull; PAN India</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Scrollable Modal Content (Scrolls smoothly if screen height is small) */}
        <div className="flex-1 overflow-y-auto overscroll-contain p-5 sm:p-6 md:p-7">
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <p className="text-xs text-slate-600 mb-2 leading-relaxed">
                Please provide your company requirements below. We provide 100% compliant manpower deployment across PAN India.
              </p>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-white border border-slate-300 text-sm focus:outline-none focus:border-[#00A8EC] transition-all"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 bg-white border border-slate-300 text-sm focus:outline-none focus:border-[#00A8EC] transition-all"
                >
                  <option value="General Manpower Solutions">General Manpower Solutions</option>
              ...
              <div>
                ...
                <textarea
                  required
                  rows={3}
                  placeholder="Describe requirement (headcount, shifts, location)..."
                  value={formData.message}
                  onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                  className="w-full px-3.5 py-2.5 bg-white border border-slate-300 text-sm focus:outline-none focus:border-[#00A8EC] transition-all resize-y min-h-[80px]"
                />
              </div>
              ...
              <div className="pt-1">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 sm:py-3.5 px-4 text-xs sm:text-sm font-bold uppercase tracking-wider text-white bg-black hover:bg-slate-900 transition-all flex items-center justify-center space-x-2 cursor-pointer shadow-md active:scale-[0.99]"
                >
                  {loading ? (
              ...
              <div className="flex items-center justify-center space-x-2 pt-1 pb-1 text-[11px] text-slate-500">
                <ShieldCheck className="w-3.5 h-3.5 text-[#08546C] shrink-0" />
